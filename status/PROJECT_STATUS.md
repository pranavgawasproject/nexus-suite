...
- `TaskDependencies` panel in `TaskDetailDialog` (list, add blocks/relates/blocked-by, remove)
- Loads `/api/dependencies` + task picker from `/api/tasks`
- Mirrors TaskComments UX patterns (border-t section, badges, toasts)
...
- **Task dependencies UI** — list/add/remove in TaskDetailDialog (blocks, blocked-by, relates)
...
- [x] **Task dependencies API** (Gantt foundation, inspired by OpenProject) â `TaskDependency` + `/api/dependencies` + TaskDetailDialog UI.
