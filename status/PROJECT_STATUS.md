...
## Track A (this run): Public API v1 GET team

**Code files changed:** `src/app/api/v1/team/route.ts`, `docs/API.md`, `status/PROJECT_STATUS.md`

- Expose org team members + departments under `/api/v1/team` (core identity, gated on tasks)
- Optional filters: `role`, `departmentId`, `limit` (max 100)
- Returns user id/name/email/role/designation/department for assignee lookups and integrations

## â Completed

- **Public API v1 POST rooms** (`/api/v1/rooms`, rooms module)
- **Public API v1 GET team** (`/api/v1/team`, core identity)
...
**Suggested build priority:** GitHub sync â self-host deploy polish â Wiki upgrade â Custom fields.
